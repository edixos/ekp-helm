...
![Version: 0.1.9](https://img.shields.io/badge/Version-0.1.9-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 1.16.5](https://img.shields.io/badge/AppVersion-1.16.5-informational?style=flat-square)
...
|------------|------|---------|
| https://kubeovn.github.io/kube-ovn/ | kube-ovn(kube-ovn-v2) | v1.16.5 |
...
|-----|------|---------|-------------|
| kube-ovn.aclSampling | object | "{}" | Experimental OVN ACL sampling for NetworkPolicy events. |
| kube-ovn.aclSampling.allowProbabilityPercent | float | `1` | Sampling probability percentage for allowed decisions. Fractional percentages are supported. |
| kube-ovn.aclSampling.appIDEstablished | int | `103` | OVN sampling application ID for established ACL sessions. |
| kube-ovn.aclSampling.appIDNew | int | `102` | OVN sampling application ID for new ACL sessions. |
| kube-ovn.aclSampling.collectorIDAllow | int | `1` | OVN sample collector ID for allowed decisions. |
| kube-ovn.aclSampling.collectorIDDefaultDeny | int | `2` | OVN sample collector ID for default-deny decisions. |
| kube-ovn.aclSampling.defaultDenyProbabilityPercent | float | `100` | Sampling probability percentage for default-deny decisions. Fractional percentages are supported. |
| kube-ovn.aclSampling.enabled | bool | `false` | Enable controller-side ACL sampling and node-local psample delivery. |
| kube-ovn.aclSampling.localGroupID | int | `142` | Linux psample multicast group ID used for local delivery. |
| kube-ovn.aclSampling.setID | int | `142` | OVN sample collector set ID shared by controller and agents. |
...
| kube-ovn.agent.resources | object | `{"limits":{"cpu":"1000m","ephemeral-storage":"1Gi","memory":"1Gi"},"requests":{"cpu":"100m","memory":"100Mi"}}` | Agent daemon resource limits & requests. ref: https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/ |
| kube-ovn.agent.serviceMonitor | object | "{}" | Agent serviceMonitor configuration. |
| kube-ovn.agent.serviceMonitor.enabled | bool | `false` | Enable the deployment of the ServiceMonitor for the agent. |
...
| kube-ovn.bgpSpeaker.extraEnv | list | `[]` | Extra environment variables to be added to kube-ovn-speaker pods. |
...
| kube-ovn.central.resources | object | `{"limits":{"cpu":"3","ephemeral-storage":"1Gi","memory":"4Gi"},"requests":{"cpu":"300m","memory":"200Mi"}}` | ovn-central resource limits & requests. ref: https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/ |
...
| kube-ovn.global | object | `{"images":{"kubeovn":{"repository":"kube-ovn","tag":"v1.16.5"}},"registry":{"address":"docker.io/kubeovn","imagePullSecrets":[]}}` | Global configuration. |
...
| kube-ovn.cni.mountConfigDirectory | string | `"/etc/cni/net.d"` | Location of the CNI configuration to be mounted inside the pod. |
| kube-ovn.cni.mountToolingDirectory | bool | `false` | Whether to mount the node's tooling directory into the pod. |
| kube-ovn.cni.nonPrimaryCNI | bool | `false` | Whether to use Kube-OVN as non-primary CNI. When set to true, Kube-OVN will not allocate/handle primary network interfaces. Interfaces are created using Network Attachment Definitions (NADs) |
...
| kube-ovn.controller.resources | object | `{"limits":{"cpu":"1000m","ephemeral-storage":"1Gi","memory":"1Gi"},"requests":{"cpu":"200m","memory":"200Mi"}}` | kube-ovn-controller resource limits & requests. ref: https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/ |
| kube-ovn.controller.serviceMonitor | object | "{}" | Controller serviceMonitor configuration. |
| kube-ovn.controller.serviceMonitor.enabled | bool | `false` | Enable the deployment of the ServiceMonitor for the Kube-OVN controller. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| kube-ovn.features | object | `{"ENABLE_ANP":false,"ENABLE_BIND_LOCAL_IP":true,"ENABLE_DNS_NAME_RESOLVER":false,"ENABLE_OVN_LB_PREFER_LOCAL":false,"LS_CT_SKIP_DST_LPORT_IPS":true,"LS_DNAT_MOD_DL_DST":true,"OVSDB_CON_TIMEOUT":3,"OVSDB_INACTIVITY_TIMEOUT":10,"SET_VXLAN_TX_OFF":false,"enableExternalVpcs":false,"enableHardwareOffload":false,"enableKeepVmIps":true,"enableLiveMigrationOptimization":true,"enableLoadbalancer":true,"enableLoadbalancerService":false,"enableNatGateways":true,"enableNetworkPolicies":true,"enableOvnInterconnections":false,"enableOvnIpsec":false,"enableSecureServing":false,"enableTproxy":false,"enableU2OInterconnections":false}` | Features of Kube-OVN we wish to enable/disable. |
| kube-ovn.features.enableExternalVpcs | bool | `false` | Enable external VPCs |
...
### OVN IC controller configuration

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| kube-ovn.ic | object | "{}" | Configuration for the OVN interconnection (IC) controller. |
| kube-ovn.ic.extraEnv | list | `[]` | Extra environment variables to be added to ovn-ic-controller pods. |
| kube-ovn.ic.nodeAffinity | object | `{"preferredDuringSchedulingIgnoredDuringExecution":[],"requiredDuringSchedulingIgnoredDuringExecution":[]}` | More information on formatting nodeAffinity can be found at https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#node-affinity |
...
| kube-ovn.monitor.resources | object | `{"limits":{"cpu":"200m","ephemeral-storage":"1Gi","memory":"200Mi"},"requests":{"cpu":"200m","memory":"200Mi"}}` | kube-ovn-monitor resource limits & requests. ref: https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/ |
| kube-ovn.monitor.serviceMonitor | object | "{}" | kube-ovn-monitor serviceMonitor configuration. |
| kube-ovn.monitor.serviceMonitor.enabled | bool | `false` | Enable the deployment of the ServiceMonitor for the kube-ovn-monitor. |
...
| kube-ovn.natGw.bgpSpeaker.image.tag | string | `"v1.16.5"` | Image tag. |
...
| kube-ovn.natGw.image.tag | string | `"v1.16.5"` | Image tag. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| kube-ovn.networkPolicies | object | "{}" | Configuration for network policies |
| kube-ovn.networkPolicies.enforcement | string | `"standard"` | Enforcement level of network policies when they get applied (can be: standard, lax). Enforcement "standard" blocks everything except what is allowed by the network policies. Enforcement "lax" is similar to "standard" with the exception that ARP/DHCPv4/DHCPv6/ICMPv4/ICMPv6 is allowed by default. This mode is useful when using Kubevirt and VMs with IPs configured via Kube-OVN's DHCP. |
...
| kube-ovn.networking.pods.mtu | int | `0` | MTU of the subnet. If set to 0, the MTU is auto-detected. |
...
| kube-ovn.ovsOvn.affinity | object | `{}` | Affinity for ovs-ovn pods. |
...
| kube-ovn.ovsOvn.dpdkHybrid.affinity | object | `{}` | Affinity for the ovs-ovn-dpdk DaemonSet. |
...
| kube-ovn.ovsOvn.dpdkHybrid.tag | string | `"v1.16.5-dpdk"` | DPDK image tag. |
...
| kube-ovn.ovsOvn.updateStrategy | object | `{"maxSurge":1,"maxUnavailable":0,"type":"RollingUpdate"}` | ovs-ovn DaemonSet update strategy. ref: https://kubernetes.io/docs/tasks/manage-daemon/update-daemon-set/#daemonset-update-strategy |
| kube-ovn.ovsOvn.upgrade | object | `{"enabled":true}` | Upgrade logic for OVS/OVN. |
...
| kube-ovn.pinger.resources | object | `{"limits":{"cpu":"200m","ephemeral-storage":"1Gi","memory":"400Mi"},"requests":{"cpu":"100m","memory":"100Mi"}}` | kube-ovn-pinger resource limits & requests. ref: https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/ |
| kube-ovn.pinger.serviceMonitor | object | "{}" | Ping daemon serviceMonitor configuration. |
| kube-ovn.pinger.serviceMonitor.enabled | bool | `false` | Enable the deployment of the ServiceMonitor for the pinger. |
...
| kube-ovn.validatingWebhook.extraEnv | list | `[]` | Extra environment variables to be added to kube-ovn-webhook pods. |
...
| kube-ovn.controller.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution | list | `[]` | - antarctica-west1 |
| kube-ovn.ic.nodeAffinity.preferredDuringSchedulingIgnoredDuringExecution | list | `[]` | - antarctica-west1 |
