...
| bgp | object | `{"advertiseMode":"DEFAULT","advertisedGroups":[],"advertisedIpRanges":[{"description":null,"range":null}],"asn":64512,"keepaliveInterval":20}` | BGP information specific to this router. |
...
| bgp.asn | int | `64512` | Local BGP Autonomous System Number (ASN). Must be an RFC6996 private ASN, either 16-bit or 32-bit. The value will be fixed for this router resource. All VPN tunnels that link to this router will have the same local ASN. |
...
helm install ekp-helm/gcp-compute-router
```
...
```bash
docker run --rm -it -w /charts -v $(pwd)/../../:/charts quay.io/helmpack/chart-testing:v3.12.0 ct lint --charts /charts/charts/gcp-compute-router --config /charts/charts/gcp-compute-router/ct.yaml
```
...
```
docker run --rm -it -v $(pwd):/apps -v pluto:/pluto alpine/helm:3.17 template gcp-compute-router . -f tests/pluto/values.yaml --output-dir /pluto
